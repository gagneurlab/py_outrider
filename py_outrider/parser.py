...
def parse_args(args_input):
    parser = argparse.ArgumentParser(description='Run py_outrider to detect aberrant events in omics data.')
    # input / output params
    parser.add_argument('-in','--input', type=str, help='Path to a file containing the input data matrix, like a gene count table. Can either be a csv file with format: rows:samples x columns:genes and 1. row and 1. column having feature and sample names, respectively; or a .h5ad file containing an existing AnnData object .' ) #, required=True )
    ...
    parser.add_argument('-o','--output', type=str, default=None, help='Filename to save results (adata object).')
    parser.add_argument('-ot','--output_type', type=str, default='h5ad', choices=['h5ad', 'csv', 'zarr'], help='File type of the output (h5ad, csv or zarr). Default: h5ad')
    parser.add_argument('-or','--output_res_table', type=str, default=None, help='Outputs results table as csv to the specifed file.')

    # outrider params
    parser.add_argument('-p','--profile', default='outrider', choices=['outrider', 'protrider', 'pca'], help='Choose which pre-defined implementation should be used. Default profile: outrider')
    parser.add_argument('-q','--encod_dim', type=int, default=None, help='Number of encoding dimensions. If not specified, runs hyperparameter optimization to determine best encoding dimension.')#, required=True)
    ...
    parser.add_argument('-i','--iterations', type=check_positive_or_zero_int, default=None, help='[predefined in profile] Number of maximal training iterations. Default: 15')
    parser.add_argument('-fl','--float_type', default=None, choices=['float32', 'float64'], help='[predefined in profile] Which float type should be used, highly advised to keep float64 which may take longer, but accuracy is strongly impaired by float32. Default: float64')
    parser.add_argument('-cpu','--num_cpus', type=check_positive_int, default=None, help='Number of cpus used. Default: 1')
    parser.add_argument('-v','--verbose', type=bool, default=None, help='Set this flag to enable printing of additional information during model fitting.')
    parser.add_argument('-s','--seed', type=int, default=None, help='Seed used for training, negative values (e.g. -1) -> no seed set. Default: 7')
    ...
    parser.add_argument('-dt','--data_trans', default=None, choices=['sf', 'log', 'none'], help='[predefined in profile] transformation function applied to preprocessed input data to create input for AE model.')
