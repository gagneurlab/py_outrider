...
class Loss_dis_neg_bin(Loss_dis_abstract):
    ...
    def tf_loss_E( e, D, b, x, x_trans, par_sample, par_meas, cov_sample, data_trans, **kwargs):
        if data_trans.__name__ =="Trans_sf":
            ...
            return Loss_dis_abstract.tf_loss_E(e=e, D=D, b=b, x=x, x_trans=x_trans, par_sample=par_sample,
                                   par_meas=par_meas, cov_sample=cov_sample,  data_trans=data_trans, **kwargs)
    ...
    def tf_loss_D_single(H, x_i, b_and_D, par_sample, par_meas, data_trans, **kwargs):
        if data_trans.__name__ =="Trans_sf":
            ...
            return Loss_dis_abstract.tf_loss_D_single(H=H, x_i=x_i, b_and_D = b_and_D, par_sample = par_sample,
                                                           par_meas=par_meas, data_trans=data_trans, **kwargs)
    ...
    def tf_loss_D(H, x , b_and_D, par_sample, par_meas, data_trans, **kwargs):
        if data_trans.__name__ =="Trans_sf":
            ...
            return Loss_dis_abstract.tf_loss_D(H=H, x=x, b_and_D = b_and_D, par_sample = par_sample,
                                                           par_meas=par_meas, data_trans=data_trans, **kwargs)
