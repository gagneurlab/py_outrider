...
class Loss_list():
    ...
    def _is_converged(self, loss_list, conv_limit, last_iter, verbose):
        """
        check on current loss_list if it is converged and has not change a certain limit the last few steps
        :param loss_list: list of values (losses) or list of list of values, e.g. [[loss], [loss of 5 col]] to
        track convergence on a measurement level
        :param conv_limit: numeric convergence criteria
        :param last_iter: number on how many steps the convergence criteria must be satisfied to be accounted converged
        ...
        if len(loss_list) > last_iter:
            ...
            if np.size(loss_list[0]) > 1:
                ...
                if verbose:
                    print(
                        f'### cols_converged: {num_converged}     cols_not_converged: {len(meas_conv) - num_converged}')
                return num_converged == len(meas_conv), str(num_converged.numpy()) + '/' + str(
                    (len(meas_conv) - num_converged).numpy())
            else:
                conv = meas_conv == True
                if verbose:
                    print(f'### loss_converged: {meas_conv}   last {last_iter} losses: {l_short}')
                return conv, str(conv)
        return False, str(False)


    def check_converged(self, conv_limit, last_iter=3, verbose=False):
        return self._is_converged(self.losses, conv_limit, last_iter, verbose)[0]
