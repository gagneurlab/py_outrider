...
def calc_adjusted_pvalues(adata, method='fdr_by'):
    ...
    """
    assert "X_pvalue" in adata.layers.keys(), 'No X_pvalue found in AnnData object, calculate pvalues first.'
...
def aberrant(adata, alpha, effect_cutoffs={}):
    ...
    assert "X_padj" in adata.layers.keys(), 'No X_padj found in AnnData object, calculate adjusted pvalues first.'
    
    aberrant = adata.layers["X_padj"] < alpha
    
    for effect_type in effect_cutoffs:
        assert "outrider_" + effect_type in adata.layers.keys(), 'Did not find {"outrider_" + effect_type} in adata.layers.keys()'
        aberrant = np.logical_and(aberrant, adata.layers["outrider_" + effect_type] > effect_cutoffs[effect_type])
