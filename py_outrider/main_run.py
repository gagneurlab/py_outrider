...
def full_run(args_input):
    ...
    if outrider_args["encod_dim"] is None:
        print_func.print_time('encod_dim is None -> run hyperparameter optimisation')
    ...
    # output
    print_func.print_time(f'Saving result AnnData object to file {args["output"]}')
...
def run_from_R_OUTRIDER(X_input, sample_anno_input, args_input):
    ...
    print_func.print_time('create adata object out of input data')
    adata = create_adata_from_arrays(X_input, sample_anno = sample_anno_input, dtype=outrider_args['float_type'])
