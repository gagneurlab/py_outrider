...
def add_noise(adata, noise_factor):
    ...
    if noise_factor > 0:
        # Add gaussian noise
        noise = np.random.normal(loc=0, scale=1, size=adata.X.shape) * noise_factor * np.nanstd(adata.X, ddof=1, axis=0)
...
def inject_outliers(adata, inj_freq=1e-3, inj_mean=3, inj_sd=1.6, **kwargs):
    ...
    X_injected = rev_trans(X_injected_trans, sf=adata.obsm["sizefactors"], trans_func=kwargs["data_trans"])
    
    # avoid inj outlier to be too strong
    max_outlier_value = np.nanmin([100 * np.nanmax(adata.layers["X_prepro"]), np.finfo(adata.layers["X_prepro"].dtype).max])
    cond_value_too_big = X_injected > max_outlier_value
    X_injected[cond_value_too_big] = adata.layers["X_prepro"][cond_value_too_big]
