...
def parse_args(args_input):
    ...
    parser.add_argument('-in','--file_meas', type=str, help='path to a measurement file, like a gene count table, format: rows:samples x columns:genes, must be comma separated, needs 1. row and 1. column to have names') #, required=True ) TODO CHANGE
    ...
    parser.add_argument('-cov','--covariates', default=None, nargs='*', help='list of covariate names in file_sa, can be columns with 1|0, numbers or strings' )
    ...
    parser.add_argument('-dis','--distribution', default=None, choices=['neg_bin', 'gaus'], help='distribution assumed for the measurement data [predefined in profile]')
    parser.add_argument('-dt','--data_trans', default=None, choices=['sf', 'log', 'none'], help='change transformation scheme for measurement data during training [predefined in profile]')
    parser.add_argument('-nf','--noise_factor', default=None, type=float, help='factor which defines the amount of noise applied for a denoising autoencoder model [predefined in profile]')
    parser.add_argument('-ld','--loss_dis', default=None, choices=['neg_bin', 'gaus'], help='loss distribution used for training [predefined in profile]')
    parser.add_argument('-pre','--prepro', default=None, choices=['none', 'sf_log'], help='preprocess data before input [predefined in profile]')
